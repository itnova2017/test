...
function displayClock() {
	...
	zn.style.paddingTop = '20px';
	zn.style.zIndex = 990;
	ctx.appendChild(zn);
	d.getElementById('clock_a').appendChild(ctx);
	for ( var i = 12; i > 0; i--) {
		d.getElementById('cnum' + i).style.top = (ctrY - offset + rad
				* Math.sin(i * pi / 6 - pi2))
				+ 'px';
		d.getElementById('cnum' + i).style.left = (ctrX - offset + rad
				* Math.cos(i * pi / 6 - pi2))
				+ 'px';
...
function updateClock() {
	...
	d.getElementById('ampm').style.top = (ypos + rad / 3) + 'px'; /*
																	 * d.getElementById('ampm').innerHTML =
																	 * theTime[1] + '<br />' +
																	 * dayname[theTime[0]];
																	 */
	...
	moveClock(hourln, 'chour', pi * theTime[2] / 6 + pi * (+now.getMinutes())
			/ 360 - pi2);
